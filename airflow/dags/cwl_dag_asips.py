...
POD_NAMESPACE = "sps"
POD_LABEL = "cwl_task"
SPS_DOCKER_CWL_IMAGE = "ghcr.io/unity-sds/unity-sps/sps-docker-cwl:2.2.0-beta-2"
...
DEFAULT_CWL_WORKFLOW = (
    "https://raw.githubusercontent.com/unity-sds/unity-sps-workflows/main/demos/echo_message_ecr.cwl"
)
...
dag = DAG(
    dag_id="cwl_dag_ecr",
    description="CWL DAG for ECR",
    dag_display_name="CWL DAG with ECR support",
    tags=["CWL", "ECR"],
...
def setup(ti=None, **context):
    ...
    # retrieve the ECR_URI for this account from the Airflow variables and pass it to the next task
    ecr_uri = Variable.get("ECR_URI")
    logging.info(f"ECR_URI={ecr_uri}")
    ti.xcom_push(key="ecr_uri", value=ecr_uri)
...
cwl_task = SpsKubernetesPodOperator(
    ...
    arguments=[
        "{{ params.cwl_workflow }}",
        "{{ params.cwl_args }}",
        "{{ti.xcom_pull(task_ids='Setup', key='ecr_uri')}}",
    ],
